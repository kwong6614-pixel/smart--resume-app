      ...
      if (isJobExperience) {
        ...
        if (isSummaryOrEducation) {
          ...
          if (colonIndex !== -1) {
            ...
            const wrapped = wrapBulletText(textToWrap, font, bodySize, contentWidth - 20);
...
export async function renderTemplate6(context: TemplateContext): Promise<Uint8Array> {
  ...
  if (contactParts.length > 0) {
    ...
    for (const line of contactLines) {
      drawCenteredTextWithOptionalLink(page, pdfDoc, line, 'LinkedIn', context.linkedin, font, CONTACT_SIZE, rgb(0.95, 0.95, 0.95), PAGE_WIDTH, contactY);
